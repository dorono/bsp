...
/***************************************************************************/
		...
		$output .=	'	<input type="text" name="setting-header_slider_posts_slides" value="' . themify_get( 'setting-header_slider_posts_slides' ) . '" class="width2" /> ' . __('number of posts to be queried', 'themify') . ' 
					...
					<p class="feature_box_posts" '.$display_posts_display.'>
						<span class="label">' . __('Content Display', 'themify') . ' </span>
								...
								foreach($display_options as $option => $value){
									if ( $value == themify_get( 'setting-header_slider_default_display' ) ) {
			...
			$output .= '<p class="feature_box_posts" '.$display_posts_display.'>
						...
						foreach($title_options as $name => $option){
							if ( $option == themify_get( 'setting-header_slider_hide_title' ) ) {
	...
	function themify_footer_slider($data=array()){
		...
		$output .=	'	<input type="text" name="setting-footer_slider_posts_slides" value="' . themify_get( 'setting-footer_slider_posts_slides' ) . '" class="width2" /> ' . __('number of posts to be queried', 'themify') . ' 
					...
					<p class="feature_box_posts" '.$display_posts_display.'>
						<span class="label">' . __('Content Display', 'themify') . ' </span>
								...
								foreach($display_options as $option => $value){
									if ( $value == themify_get( 'setting-footer_slider_default_display' ) ) {
										$output .= '<option value="'.$value.'" selected="selected">'.$option.'</option>';
									} else {
										$output .= '<option value="'.$value.'">'.$option.'</option>';
									}
			...
			$output .= '<p class="feature_box_posts" '.$display_posts_display.'>
						...
						';
						foreach($title_options as $name => $option){
							if ( $option == themify_get( 'setting-footer_slider_hide_title' ) ) {
								$output .= '<option value="'.$option.'" selected="selected">'.$name.'</option>';
							} else {
								$output .= '<option value="'.$option.'">'.$name.'</option>';
							}
		...
		$output .= '</div>
					...
					<p>
						...
						for($x=1;$x<=7;$x++){
							if ( themify_get( 'setting-footer_slider_visible' ) == $x ) {
								$output .= '<option value="'.$x.'" selected="selected">'.$x.'</option>';	
							} else {
								$output .= '<option value="'.$x.'">'.$x.'</option>';	
							}
			...
			$output .=	'</select> <small>' . __('(# of slides visible at the same time)', 'themify') . '</small>
					...
					<span class="label">' . __('Auto Play', 'themify') . '</span>
								...
								';
							foreach($auto_options as $option){
								if ( $option == themify_get( 'setting-footer_slider_auto' ) ) {
									$output .= '<option value="'.$option.'" selected="selected">'.$option.'</option>';
								} else {
									$output .= '<option value="'.$option.'">'.$option.'</option>';
								}
			...
			$output .= '
					...
					<span class="label">' . __('Scroll', 'themify') . '</span>
								...
								';
							foreach($scroll_options as $option){
								if ( $option == themify_get( 'setting-footer_slider_scroll' ) ) {
									$output .= '<option value="'.$option.'" selected="selected">'.$option.'</option>';
								} else {
									$output .= '<option value="'.$option.'">'.$option.'</option>';
								}
			...
			$output .= '
					...
					<p>
						...
						foreach($speed_options as $name => $val){
							if ( $val == themify_get( 'setting-footer_slider_speed' ) ) {
								$output .= '<option value="'.$val.'" selected="selected">'.$name.'</option>';	
							} else {
								$output .= '<option value="'.$val.'">'.$name.'</option>';	
							}	
			...
			$output .= '</select>
					...
					<p>
						...
						foreach($wrap_options as $name => $value){
							if ( themify_get( 'setting-footer_slider_wrap' ) == $value ) {
								$output .= '<option value="'.$value.'" selected="selected">'.$name.'</option>';	
							} else {
								$output .= '<option value="'.$value.'">'.$name.'</option>';	
							}
	...
	function themify_footer_slider_action($data=array()){
		$wrap = 'false';
		if(themify_get( 'setting-footer_slider_wrap' ) == 'yes'){
			$wrap = 'true';	
		}
		if(themify_get( 'setting-footer_slider_visible' ) == ''){
			$visible = '4';
		} else {
			$visible = themify_get( 'setting-footer_slider_visible' );
		}
		if('' == themify_get( 'setting-footer_slider_speed' )){
			$speed = '2500';
		} else {
			$speed = themify_get( 'setting-footer_slider_speed' );
		}
		if(themify_get( 'setting-footer_slider_scroll' ) == ''){
			$scroll = 1;	
		} else {
			$scroll = themify_get( 'setting-footer_slider_scroll' );
		}
		if(themify_get( 'setting-footer_slider_auto' ) == ''){
			$auto = 0;	
		} else {
			$auto = themify_get( 'setting-footer_slider_auto' );
		}
		if( '0' == $auto )
			$play = 'false';
		else
